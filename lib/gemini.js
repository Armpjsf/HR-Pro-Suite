...
export function analyzeIntent(message) {
  ...
  // ถามวันลา (ตรวจก่อนเรื่องขอเอกสาร)
  if (msg.match(/วันลา|ลาเหลือ|ลาคงเหลือ|leave.*balance|เหลือกี่วัน|พักร้อน.*เหลือ/)) {
  ...
  // ถามระเบียบ/Policy
  if (msg.match(/ระเบียบ|กฎ|นโยบาย|policy|สวัสดิการ|benefit|แต่งกาย|การลา|ได้กี่วัน/)) {
 ...
 */
function findRelevantDocuments(intent, userRole, employeeId) {
 ...
 * ดึงข้อมูลพนักงาน
 */
function getEmployeeData(employeeId, dataType) {
  const data = getEmployeeRecord(employeeId);
  if (!data) return null;
...
export async function generateResponse(message, user) {
  ...
  if (intent.type === 'document_request') {
    const docs = findRelevantDocuments(intent, user.role, user.employeeId);
    if (docs.length > 0) {
  ...
  if (intent.type === 'data_query') {
    if (intent.dataType === 'leave') {
      ...
      const leaveData = getEmployeeData(targetEmployeeId, 'leave');
    ...
    if (intent.dataType === 'profile') {
      const profileData = getEmployeeData(user.employeeId);
      if (profileData) {
        return {
  ...
  const relevantDocs = findRelevantDocuments(intent, user.role, user.employeeId);
