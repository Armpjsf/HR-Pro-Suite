...
import { currentLeaveYear, formatLeaveBalanceMap, legacyLeaveRows } from './leave-balances';
...
export async function updateDocumentContent(docId, content) {
  const { error } = await supabase
    .from('documents')
    .update({ content: content || '', updated_at: new Date().toISOString().split('T')[0] })
    .eq('id', docId);
  throwIfError(error, 'updateDocumentContent');
}
...
export async function getEmployeeRecord(employeeId) {
  ...
  const [
    ...
    { data: recRow, error: recErr },
    { data: leaveTypes, error: ltErr },
    { data: balanceRows, error: bErr },
  ] = await Promise.all([
  ...
  throwIfError(ltErr, 'getEmployeeRecord (leave types)');
  throwIfError(bErr, 'getEmployeeRecord (leave balances)');

  if (!recRow) return null;
  const rows = balanceRows?.length ? balanceRows : legacyLeaveRows(employeeId, recRow, year);
  ...
  return {
    ...
    leave: formatLeaveBalanceMap(rows, leaveTypes || []),
  };
}
